...
async function refreshCctvSources() {
  ...
  const [austinResult, caltransResult, tflResult] = await Promise.allSettled([
    needsLiveSources ? loadAustinSourcesFromOpenData() : Promise.resolve([]),
    needsLiveSources ? loadCaltransSourcesFromOpenData() : Promise.resolve([]),
    needsLiveSources && tflEnabled ? loadTflSourcesFromOpenData() : Promise.resolve([]),
  ]);
  const fromAustin = austinResult.status === 'fulfilled' ? austinResult.value : [];
  const fromCaltrans = caltransResult.status === 'fulfilled' ? caltransResult.value : [];
  const fromTfl = tflResult.status === 'fulfilled' ? tflResult.value : [];

  // OSM mapped cameras are deliberately NOT in this catalog: they are loaded per
  // viewport through /api/osm-cameras and merged into the live layer, so the
  // startup catalog stays the set of camera packs that actually carry frames.
