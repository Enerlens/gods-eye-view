...
| Transit FR 🚌 | transport.data.gouv.fr GTFS-Realtime vehicle positions (~150 French networks; observed footprints in `config/pan_gtfs_rt_feeds.json`), plus each network's TripUpdates feed and its static GTFS's GeoJSON conversion for the selected vehicle's line (`config/pan_gtfs_static.json`) | `src/data/transitFrance.js`, `src/data/transitRouteView.js` | `/api/transit-fr/vehicles`, `/api/transit-fr/feeds`, `/api/transit-fr/trip` | 15s fleet, viewport-driven below ~300 km; 25s for the selected run |
