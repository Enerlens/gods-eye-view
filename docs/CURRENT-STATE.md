...
| Bornes IRVE 🇫🇷 🔌 | *fichier consolidé des bornes IRVE* (transport.data.gouv.fr, via ODRÉ) — 231,079 points de charge folded to one site per coordinate. Three regimes by view span: 96 départements in quantile bins (≥ 9.5° lat), a grid-thinned maillage of real positions (9.5°–0.35°), every site with full detail (≤ 0.35°) | `src/data/irveFrance.js`, `src/data/irveFeed.js`, `src/data/irveDepartements.js`, `src/data/irveMesh.js` | `/api/irve-fr/sites`, `/api/irve-fr/departements`, `/api/irve-fr/mesh`, `/api/irve-fr/status` | 30 min (viewport TTL 6 h, national TTL 24 h; upstream consolidation is daily) |
