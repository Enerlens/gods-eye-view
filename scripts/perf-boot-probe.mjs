...
for (let r = 0; r < runs; r++) {
  ...
  await page.evaluate(() => {
    window.__probe = { firstRender: null };
  ...
  await new Promise((res) => setTimeout(res, layers.length ? 15_000 : 8_000));
...
const summary = {
  url, cpu, net, runs, windowMs, at, layers,
...
if (asJson) {
  ...
  console.log(
    `\nMEDIAN cpu×${cpu} net=${net} at=${at}${layers.length ? ` layers=${layers.join('+')}` : ''}`
    + ` viewer=${f(summary.viewerMs)}ms firstRender=${f(summary.firstRenderMs)}ms`
    + ` app=${f(summary.appMB, 2)}MB/${f(summary.appReqs)}req`
    + ` window=${f(summary.MB, 2)}MB/${f(summary.reqs)}req heap=${f(summary.heapMB)}MB`
