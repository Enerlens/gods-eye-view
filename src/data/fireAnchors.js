...
//
// A second finding (2026-09-10) added the PROVISIONAL floor: the DEM answers
// over the network, and a detection painted before it answers sat at ellipsoid
// 0 — hundreds of metres under its own ground — where it slid over the
// landscape with every camera move. Cold cells now get one budgeted
// `scene.sampleHeight` read of the surface actually being drawn, so a sprite
// is on the ground from its first frame; the DEM still refines it.
//
// That store lives in `provisionalFloor.js`, not here, because a ~111 m cell
// of ground is the same ground whichever layer asks — the French shared
// vehicles hit the identical bug on the identical cells. This module keeps the
// FIRE-specific half: the lift, and the DEM warm chain below.
import { cachedGroundFloor, resolveGroundFloorCells } from './groundFloor.js';
import {
  PROVISIONAL_FILL_KM,
  PROVISIONAL_MAX_CAMERA_M,
  PROVISIONAL_MAX_CELLS,
  PROVISIONAL_MAX_M,
  PROVISIONAL_MAX_PROBES,
  PROVISIONAL_MIN_M,
  provisionalFloor,
...
//
// The store, the probe budget and the plausibility band are all in
// `provisionalFloor.js`; the names below are this layer's stable surface onto
// it, kept because `fireAnchors.test.mjs` and `firmsHeatmap.js` speak them.

/** @see PROVISIONAL_MAX_PROBES */
export const FIRE_PROVISIONAL_MAX_PROBES = PROVISIONAL_MAX_PROBES;
/** @see PROVISIONAL_MAX_CAMERA_M */
export const FIRE_PROVISIONAL_MAX_CAMERA_M = PROVISIONAL_MAX_CAMERA_M;
/** @see PROVISIONAL_MIN_M */
export const FIRE_PROVISIONAL_MIN_M = PROVISIONAL_MIN_M;
/** @see PROVISIONAL_MAX_M */
export const FIRE_PROVISIONAL_MAX_M = PROVISIONAL_MAX_M;
/** @see PROVISIONAL_FILL_KM — a fire complex stands on one hillside. */
export const FIRE_PROVISIONAL_FILL_KM = PROVISIONAL_FILL_KM;
/** @see PROVISIONAL_MAX_CELLS */
export const FIRE_PROVISIONAL_MAX_CELLS = PROVISIONAL_MAX_CELLS;
...
export function provisionalFireFloor(lat, lon) {
  return provisionalFloor(lat, lon);
}

/**
 * Fill the provisional store for a rendered detection set.
 *
 * Call it ONCE per render pass, before the anchors are read. Returns the
 * probes actually spent and how many cells a LATER pass could still do better
 * on — the caller's cue that coming back once the tiles land is worth a
 * re-render, because grounded is not the same as grounded WELL.
 *
...
export function sampleFireAnchorFloors(scene, points) {
  return sampleProvisionalFloors(scene, points, { fillKm: FIRE_PROVISIONAL_FILL_KM });
}
...
/** Test hook: drops the batch chain and the shared provisional store (the DEM
