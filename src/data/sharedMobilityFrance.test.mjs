...
function vehicleRecord(overrides = {}) {
  ...
  return {
    ...
    type: 'vehicle',
    object,
    system: { id: 'gbfs-84153', name: 'Lime Paris', licence: 'Licence Ouverte 2.0', ...(overrides.system || {}) },
    position: Cesium.Cartesian3.fromDegrees(object.lon, object.lat, 12),
    point: { color: null, pixelSize: 0, show: true },
    baseColor: vehicleKindColor(object.kind),
    baseSize: 6,
  };
}
...
function stationRecord(overrides = {}) {
  ...
  return {
    ...
    type: 'station',
    object,
    system: { id: 'gbfs-1', name: 'Naolib Nantes', licence: 'ODbL 1.0', ...(overrides.system || {}) },
    position: Cesium.Cartesian3.fromDegrees(object.lon, object.lat, 12),
    point: { color: null, pixelSize: 0, show: true },
...
test('every vehicle kind has a distinct tint and a readable label', () => {
  const kinds = ['bike', 'ebike', 'scooter', 'moped', 'car'];
  const colors = kinds.map(vehicleKindColor);
...
test('the selected entry takes the protected lane, and the source is a static one', () => {
  ...
  assert.equal(entry.title, 'E-bike');
...
test('selecting and clearing drives the real host seam and restores the point', () => {
  ...
  assert.equal(record.point.color.toCssHexString(), vehicleKindColor('ebike'));
  assert.equal(record.point.pixelSize, record.baseSize);
});

test('the row legend counts what is on screen and explains the merged bays', () => {
  _setSharedMobilityStateForTest({
  ...
  assert.deepEqual(legend.map((item) => [item.label, item.count]), [['E-bike', 2], ['Scooter', 1], ['Stations', 1]]);
