// src/data/radioTuner.js
//
// The radio tuner dial's geometry — slot snapping, pointer mapping and the
// virtual tick tape.
//
// WHY IT LIVES APART FROM `radio.js`. These four are pure arithmetic over a
// station count and a pixel width; the layer they used to sit in is 105 kB of
// Cesium primitives, audio plumbing and a station catalogue. `ui.js` draws the
// dial and needs nothing else from that file, so importing them from the layer
// pinned all 105 kB into the boot chunk for a handful of `Math.round` calls.
// `radio.js` re-exports them, so the layer's own test surface is unchanged.
