...
test('the shipped ENVIRONMENT template offers every key the panel does', async () => {
  // The panel uncomments a line IN PLACE when one exists and appends when it
  // does not. A registry entry missing from this template still works — it just
  // lands at the bottom, outside the curated order a Pinokio user reads. Worth
  // one assertion so the template cannot silently fall behind the registry.
  const { readFileSync } = await import('node:fs');
  const { knownKeySetupEnvVars } = await import('./keySetupCore.mjs');
  const template = readFileSync(new URL('../pinokio/_ENVIRONMENT', import.meta.url), 'utf8');
  for (const name of knownKeySetupEnvVars()) {
    assert.match(template, new RegExp(`^# ${name}=$`, 'm'), `${name} is missing from pinokio/_ENVIRONMENT`);
  }
});
