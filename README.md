...
| ≋ **Capteurs trafic** 🇫🇷 | The state's own counting loops, and what they measured: ~840 placeable stations reporting **average speed and hourly flow** every six minutes, coloured by either one on a chip. Deliberately not a congestion map — QTV publishes nothing to compare a speed *against*, so the legend reads in km/h and never in words like *fluide*. Two readings it does make: a station counting **vehicles at zero km/h** is a queue, and a `0` computed from **zero samples** is not a zero at all — one station published 7 114 véh/h beside a 0,0 km/h built from no measurement, and it draws grey. Every card carries the reading's **age**, because a QTV value is already ~12 minutes old when it reaches you. No key | Bison Futé / QTV-DIR | 🟢 |
